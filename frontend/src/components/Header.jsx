...
const ENGINE_ACTIVE_THRESHOLD_MS = 15 * 60 * 1000 // 15 minutes
...
export default function Header({ connected, lastUpdate, onMobileOpen }) {
  ...
  const [engineActive, setEngineActive] = useState(false) // true = active (< 15 min), false = idle
  ...
  useEffect(() => {
    const checkEngine = () => {
      axios.get(`${API}/api/agents`).then((r) => {
        ...
        setEngineActive(Date.now() - latest < ENGINE_ACTIVE_THRESHOLD_MS)
      }).catch(() => setEngineActive(false))
    }
    checkEngine()
    const interval = setInterval(checkEngine, 60000)
  ...
  return (
    <header className="header">
      ...
      <div className={`engine-indicator ${engineActive ? 'engine-indicator--active' : 'engine-indicator--idle'}`}>
  <span className={`engine-dot ${engineActive ? 'engine-dot--active' : 'engine-dot--idle'}`} />
  <span className="engine-emoji">⚡</span>
  <span className="engine-label">{engineActive ? 'Engine Active' : 'Engine Idle'}</span>
</div>
